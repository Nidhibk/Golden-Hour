...
def msg():
    ...
    if(select == 1 or select == 2):
        ...
        if (e1.index("end") == 0):
            ...
            mb.showinfo('Success', 'Registration done successfully for the patient '+ course)
    else:
...
def save():
    ...
    s='\n'+'\t'+e1.get()+'\t'+e2.get()+'\t '+e3.get()+'\t'+ '\t'+gender+' \t'+course
    ...
    with open('Regfile.csv', 'a') as fs:
        ...
        w.writerow([ e1.get(),e2.get(),e3.get(),gender,course])
        fs.close()
...
l1 = Label(root, text="Patient Registration form",width=25,font=("times",20,"bold"),bg='blue',fg='white')
l1.place(x=70,y=50)
...
l7 = Label(root, text="Select reason for admission",width=20,font=("times",12,"bold"),anchor="w",bg='grey')
l7.place(x=70,y=370)

# create a dropdown menu with the OptionMenu widget
cvar = StringVar()
cvar.set("Select reason for admission")
option = ("heart related", "fever", "cold","covid")
