...
import time
from  _system_facial_recognition.settings import BASE_DIR
import os
...
#used
def hourvdatepatient(present_qs,time_qs,admin=True):
    ...
    plt.tight_layout()
    if(admin):
        plt.savefig('./recognition/static/recognition/img/ _graphs/hours_vs_date/1.png')
        plt.close()
    else:
        plt.savefig('./recognition/static/recognition/img/ _graphs/ _login/1.png')
        plt.close()
    return qs
    

#used
def hourvdatepatientgiven(present_qs,time_qs):
    register_matplotlib_converters()
    df_hours=[]
    df_break_hours=[]
    df_username=[]
    ...
    rcParams.update({'figure.autolayout': True})
    plt.tight_layout()
    plt.savefig('./recognition/static/recognition/img/ _graphs/hours_vs_ /1.png')
    plt.close()
    return qs
...
def countvdate():
    ...
    df["Number of patients"]=emp_cnt_all
    
    
    sns.lineplot(data=df,x='date',y='Number of patients')
    plt.savefig('./recognition/static/recognition/img/ _graphs/this_week/1.png')
    plt.close()
...
def lastweekcountvdate():
    ...
    df=pd.DataFrame()
    df["date"]=str_dates_all
    df["ptnt_count"]=emp_cnt_all
    

    
    
    sns.lineplot(data=df,x='date',y='ptnt_count')
    plt.savefig('./recognition/static/recognition/img/ _graphs/last_week/1.png')
    plt.close()
...
def dashboard(request):
    if(request.user.username=='admin'):
        ...
        return render(request,'recognition/patientdashboard.html')
...
def add_photos(request):
    ...
    if request.method=='POST':
        ...
        if username_present(username):
            ...
            messages.warning(request, f'No such username found. Please register   first.')
...
def mark_your_ (request):
    ...
    while(True):
        ...
        #Showing the image in another window
        #Creates a window with window name "Face" and with the image img
        cv2.imshow("face Recognition - press q exit",frame)
        #Before closing it we need to give a wait command, otherwise the open cv wont work
        # @params with the millisecond of delay 1
        #cv2.waitKey(1)
        #To get out of the loop
    ...
    # destroying all the windows
    cv2.destroyAllWindows()
    updatepatient(present)
    
    return redirect('home')



def mark_your_ _out(request):
    ...
    while(True):
        ...
        #Showing the image in another window
        #Creates a window with window name "Face" and with the image img
        cv2.imshow("Mark  - Out - Press q to exit",frame)
        #Before closing it we need to give a wait command, otherwise the open cv wont work
        # @params with the millisecond of delay 1
        #cv2.waitKey(1)
        #To get out of the loop
...
@login_required
def view_ _home(request):
    total_num_of_emp=totalpatients()
    emp_present_today=patientsenteredtoday()
    countvdate()
    lastweekcountvdate()
    return render(request,"recognition/viewpatientsentrys.html", {'total_num_of_emp' : total_num_of_emp, 'emp_present_today': emp_present_today})


@login_required
def view_ _date(request):
    if request.user.username!='admin':
        return redirect('not-authorised')
    qs=None
    time_qs=None
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if(len(time_qs)>0 or len(present_qs)>0):
                qs=hourvdatepatientgiven(present_qs,time_qs)


                return render(request,'recognition/viewpatientsentryssss.html', {'form' : form,'qs' : qs })
            else:
                messages.warning(request, f'No records for selected date.')
                return redirect('view- -date')
        ...
        if form.is_valid():
            ...
            if username_present(username):
                ...
                if date_to < date_from:
                    ...
                    if (len(time_qs)>0 or len(present_qs)>0):
                        qs=hourvdatepatient(present_qs,time_qs,admin=True)
                        return render(request,'recognition/viewentrypatient.html', {'form' : form, 'qs' :qs})
                    else:
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if date_to < date_from:
                    ...
                    if (len(time_qs)>0 or len(present_qs)>0):
                        qs=hourvdatepatient(present_qs,time_qs,admin=False)
                        return render(request,'recognition/viewmyentrypatient.html', {'form' : form, 'qs' :qs})
                    else:
